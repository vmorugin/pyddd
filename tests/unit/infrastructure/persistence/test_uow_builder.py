...
class TestUowBuilder:
    async def test_insert_to_storage(self):
        ...
        assert await storage.select() == (f.FakeRow(reference=aggr.reference, value="test"),)
    ...
    async def test_locker(self):
        ...
        class RepoBuilder(IRepositoryBuilder[f.AbstractRepository]):
            async def __call__(self, _uow: IUnitOfWorkCtxMgr[f.AbstractRepository, UUID]) -> f.AbstractRepository:
    ...
    async def test_fail_enter_to_context_if_repository_build_return_not_valid_instance(self):
        class RepoBuilder(IRepositoryBuilder[int]):
            async def __call__(self, _uow: IUnitOfWorkCtxMgr[int, UUID]) -> f.AbstractRepository:
                return 1  # noqa
