...
from pyddd.infrastructure.persistence.abstractions import IRepository
...
class Storage:
    ...
    async def select(self, reference: UUID = None, value: str = None) -> tuple[FakeRow]:
        ...
        if reference is None and value is None:
            ...
            await asyncio.sleep(0.001)
            return tuple(
                FakeRow(**row) for row in self._storage if (reference == row["reference"] or value == row["value"])
            )

    async def _get_row(self, reference: UUID):
        await asyncio.sleep(0.001)
        return next((row for row in self._storage if row["reference"] == reference), None)
