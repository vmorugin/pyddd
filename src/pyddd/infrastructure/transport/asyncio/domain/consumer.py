...
class MessageConsumer(IMessageConsumer):
    ...
    def set_application(self, application: IApplication):
        application.subscribe(
            ApplicationSignal.BEFORE_RUN, listener=self._before_run_handler
        )
        application.subscribe(
            ApplicationSignal.AFTER_RUN, listener=self._after_run_handler
        )
        application.subscribe(
            ApplicationSignal.BEFORE_STOP, listener=self._before_stop_handler
        )
        application.subscribe(
            ApplicationSignal.AFTER_STOP, listener=self._after_stop_handler
        )
    ...
    async def _before_stop_handler(
        self, _signal: ApplicationSignal, _app: IApplication
    ):
