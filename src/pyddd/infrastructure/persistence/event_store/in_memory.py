...
class InMemoryEventStore(IEventStore):
    def __init__(self, memory: dict[str, list[t.Union[SnapshotABC, ISourcedEvent]]]):
        self._memory = memory
    ...
    def _get_or_create_event_stream(self, stream_name: str) -> list[ISourcedEvent]:
        ...
        stream = self._memory.get(event_stream_name)
        if stream is None:
            stream = []
            self._memory[event_stream_name] = stream
        return stream
    ...
    def _get_or_create_snapshot_stream(self, stream_name: str) -> list[SnapshotABC]:
        ...
        stream = self._memory.get(snapshot_stream_name)
        if stream is None:
            stream = []
            self._memory[snapshot_stream_name] = stream
        return stream
